...
public class DiskCache {
    ...
    public void save(String bitmapUrl, Bitmap bitmap) {
        //在缓存目录下创建缓存文件
        File cacheFile = new File(getCacheFileDir(cacheFileDirName), bitmapUrl);
        //创建图片缓存文件
        if(!cacheFile.exists()) {
            try {
    ...
    public Bitmap getBitmapFromDiskCache(String cacheFileName) {
        ...
        Bitmap bitmap;
        if(cacheFileDirName == null) {
            return null;
        }
        File cacheFile = new File(getCacheFileDir(cacheFileDirName), cacheFileName);
        try {
